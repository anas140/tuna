    ...
    <div class="row">
      <div class="col-12">
        <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form id="task_create_form" method="POST" action="{{ route('task.update', $task->id) }}">
                @csrf()
                @method('PUT')
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select name="user" class="form-control @error('user') is-invalid @enderror" >
                      @foreach($users as $user)
                          <option value="{{$user->id}}" @if($task->user->id == $user->id) selected="selected" @endif> {{ $user->name }}</option>
                      @endforeach
                    </select>
                    @error('user')
                      <span class="error invalid-feedback">{{ $message }}</span>
